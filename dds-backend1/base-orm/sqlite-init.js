// Importa el módulo "aa-sqlite", que permite interactuar con una base de datos SQLite de manera asincrónica.
const db = require("aa-sqlite");

async function CrearBaseSiNoExiste() {
  // Abre la base de datos. Si el archivo especificado (pymes.db) no existe, se crea automáticamente.
  // Este archivo se almacena en la carpeta .data.
  await db.open("./.data/pymes.db");
  //await db.open(process.env.base); // Esto se puede usar para abrir una base desde una variable de entorno, pero está comentado.

  let existe = false; // Variable para determinar si las tablas ya existen.
  let res = null; // Variable para almacenar el resultado de las consultas.

  // Verifica si la tabla "usuarios" existe consultando el esquema de SQLite.
  res = await db.get(
    "SELECT count(*) as contar FROM sqlite_schema WHERE type = 'table' and name= 'usuarios'",
    []
  );
  
  // Si "contar" es mayor que 0, significa que la tabla ya existe.
  ...
  if (!existe) {
    ...
    // Inserta registros iniciales en la tabla "usuarios" (usuarios 'admin' y 'juan' con roles diferentes).
    await db.run(
      "insert into usuarios values (1,'admin','123','admin'),(2,'juan','123','member');"
    );
  }

  // Repite el mismo proceso para la tabla "articulosfamilias".
  existe = false;
  res = await db.get(
  ...
  // Si la tabla "articulosfamilias" no existe, la crea.
  if (!existe) {
    await db.run(
    ...
    // Inserta registros iniciales en la tabla "articulosfamilias".
    await db.run(
  ...
  // Repite el mismo proceso para la tabla "articulos".
  existe = false;
  let sql =
  ...
  // Si la tabla "articulos" no existe, la crea.
  if (!existe) {
    await db.run(
    ...
    // Inserta registros iniciales en la tabla "articulos".
