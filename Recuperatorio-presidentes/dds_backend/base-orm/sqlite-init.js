...
async function CrearBaseSiNoExiste() {
  ...
  if (!existe) {
    ...
    await db.run(`INSERT INTO artistas (NombreCompleto, Edad, FechaNacimiento) VALUES 
    ('Leonardo DiCaprio', 48, '1974-11-11'),
    ('Tom Hanks', 67, '1956-07-09'),
    ('Meryl Streep', 73, '1949-06-22'),
    ('Denzel Washington', 68, '1954-12-28'),
    ('Julia Roberts', 55, '1967-10-28'),
    ('Tom Cruise', 60, '1962-07-03'),
    ('Harrison Ford', 81, '1942-07-13'),
    ('Mark Hamill', 72, '1951-09-25'),
    ('Carrie Fisher', 67, '1956-10-21'),
    ('Daniel Day-Lewis', 65, '1957-04-29'),
    ('Robert De Niro', 79, '1943-08-17'),
    ('Al Pacino', 83, '1940-04-25'),
    ('Jack Nicholson', 85, '1938-04-28'),
    ('Anthony Hopkins', 85, '1937-12-31'),
    ('Dustin Hoffman', 85, '1937-08-08'),
    ('Gene Hackman', 92, '1930-01-30'),
    ('Robert Redford', 85, '1936-08-18'),
    ('Paul Newman', 83, '1925-01-26'),
    ('Marlon Brando', 80, '1924-04-03'),
    ('James Dean', 53, '1931-02-08'),
    ('Spencer Tracy', 70, '1900-04-05');`
    )
  }

  existe = false;
  sql = "SELECT count(*) as contar FROM sqlite_schema WHERE type = 'table' and name = 'presidentes'";
  res = await db.get(sql, []);
  if (res.contar > 0) existe = true;
  if (!existe) {
    await db.run(`
    CREATE TABLE presidentes (
      IdPresidente INTEGER PRIMARY KEY AUTOINCREMENT,
      Nombre TEXT NOT NULL,
      FechaPresidencia DATE NOT NULL,
      Pais TEXT NOT NULL,
      DuracionMandatoAños INTEGER NOT NULL
    );
  `);
    console.log("tabla presidentes creada!");
    await db.run(`
    INSERT INTO presidentes (Nombre, FechaPresidencia, Pais, DuracionMandatoAños) VALUES
      ('ABRAHAM LINCOLN', '1861-03-04', 'ESTADOS UNIDOS', 4),
      ('NAPOLEÓN BONAPARTE', '1804-05-18', 'FRANCIA', 15),
      ('GEORGE WASHINGTON', '1789-04-30', 'ESTADOS UNIDOS', 8),
      ('VLADIMIR PUTIN', '2000-05-07', 'RUSIA', 22),
      ('XI JINPING', '2013-03-14', 'CHINA', 9),
      ('ANGELA MERKEL', '2005-11-22', 'ALEMANIA', 16),
      ('JACOBO ZUMA', '2009-05-09', 'SUDÁFRICA', 8),
      ('LEE KUAN YEW', '1959-06-05', 'SINGAPUR', 31),
      ('MAHATMA GANDHI', '1947-08-15', 'INDIA', 3),
      ('SALVADOR ALLENDE', '1970-11-03', 'CHILE', 3),
      ('JUAN DOMINGO PERÓN', '1946-06-04', 'ARGENTINA', 7),
      ('FIDEL CASTRO', '1959-07-26', 'CUBA', 49),
      ('MAO ZEDONG', '1949-10-01', 'CHINA', 26),
      ('JOSEPH STALIN', '1922-12-06', 'UNIÓN SOVIÉTICA', 29),
      ('BENITO MUSSOLINI', '1922-10-29', 'ITALIA', 21),
      ('ADOLF HITLER', '1933-01-30', 'ALEMANIA', 12),
      ('KIM IL-SUNG', '1948-09-09', 'COREA DEL NORTE', 46),
      ('LEOPOLDO SENGHOR', '1960-09-05', 'SENEGAL', 20),
      ('JULIUS NYERERE', '1964-12-09', 'TANZANIA', 14),
      ('JAWAHARLAL NEHRU', '1947-08-15', 'INDIA', 17),
      ('GAMAL ABDEL NASSER', '1956-06-22', 'EGIPTO', 13),
      ('SUKARNO', '1945-08-18', 'INDONESIA', 21),
      ('LEE MYUNG-BAK', '2008-02-25', 'COREA DEL SUR', 5),
      ('PARK CHUNG-HEE', '1963-12-17', 'COREA DEL SUR', 16),
      ('KIM DAE-JUNG', '1998-02-25', 'COREA DEL SUR', 5),
      ('ROH MOO-HYUN', '2003-02-25', 'COREA DEL SUR', 5),
      ('LEE MYUNG-BAK', '2008-02-25', 'COREA DEL SUR', 5),
      ('PARK GEUN-HYE', '2013-02-25', 'COREA DEL SUR', 5),
      ('MOON JAE-IN', '2017-05-10', 'COREA DEL SUR', 5);
  `);
  }
  // cerrar la base
  db.close();
